"""
Client for Groq API (fast Llama inference)
Alternative: Use HuggingFace for local Llama models
"""
import os
import time
...
class GroqLlamaClient:
    """Client for Llama via Groq API (fast and free tier available)"""
    
    # Groq pricing (FREE tier: 30 req/min, 14400 tokens/min)
    # Paid tier is very cheap
    PRICING = {
    ...
    def __init__(
        ...
        print(f" Groq Llama client initialized: {model}")
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert Python programmer. Generate clean, efficient, well-documented Python code."
                    },
                    {
                        "role": "user",
                        "content": prompt
    ...
    def _calculate_cost(self, prompt_tokens: int, completion_tokens: int) -> float:
        ...
        pricing = self.PRICING.get(self.model_name, self.PRICING["llama-3.1-8b-instant"])
